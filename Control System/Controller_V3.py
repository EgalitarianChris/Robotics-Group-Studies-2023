...
def main():
     ...
     # model = PPO.load("test_PPO_model_data") - load the trained Machine Learning Agent here
      ...
      while True:
         ...
         action = get_action_DUMMY() # - dummy function to send example actionsNao_info = get_NAO_info(TCP_connection)
         print('action:')
         print(action)
...
def send(TCP_connection, action):
  ...
  # TCP_connection.recv(1024)
   ...
   TCP_connection.sendall(pickle.dumps(action, protocol=0))# Sends back the actions
   print('action sent')
